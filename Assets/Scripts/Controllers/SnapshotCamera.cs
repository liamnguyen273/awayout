...
#pragma warning restore 0649
    ...
    public static FileInfo SaveJPG(byte[] bytes, string filename = "", string directory = "")
    {
        directory = directory != "" ? Directory.CreateDirectory(directory).FullName : Directory.CreateDirectory(Path.Combine(Application.dataPath, "../Snapshots")).FullName;
        filename = filename != "" ? SanitizeFilename(filename) + ".jpg" : System.DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss-ffff") + ".jpg";
        string filepath = Path.Combine(directory, filename);
        File.WriteAllBytes(filepath, bytes);
        return new FileInfo(filepath);
    }

    public static string GetPathWithFileName(string filename = "", string directory = "")
    {
        directory = directory != "" ? Directory.CreateDirectory(directory).FullName : Directory.CreateDirectory(Path.Combine(Application.dataPath, "../Snapshots")).FullName;
        filename = filename != "" ? SanitizeFilename(filename) + ".png" : System.DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss-ffff") + ".png";
        string filepath = Path.Combine(directory, filename);
    ...
    public void SavePNG(Texture2D tex, string filename, string directory, Action callback)
    {
        byte[] bytes = tex.EncodeToPNG();
        StartCoroutine(RoutineSavePNG(bytes, filename, directory, callback));

    }

    private IEnumerator RoutineSavePNG(byte[] bytes, string filename, string directory, Action callback)
    {
        directory = directory != "" ? Directory.CreateDirectory(directory).FullName : Directory.CreateDirectory(Path.Combine(Application.dataPath, "../Snapshots")).FullName;
        filename = filename != "" ? SanitizeFilename(filename) + ".png" : System.DateTime.Now.ToString("yyyy-MM-dd-HH-mm-ss-ffff") + ".png";
        string filepath = Path.Combine(directory, filename);
    ...
    public static FileInfo SaveJPG(Texture2D tex, string filename = "", string directory = "")
    {
        return SaveJPG(tex.EncodeToJPG(), filename, directory);
    }
    #endregion
    ...
    public Texture2D TakePrefabSnapshot(GameObject prefab, Color backgroundColor, Vector3 positionOffset, Quaternion rotation, Vector3 scale, int width = 128, int height = 128)
    {
        ...
        // Take a snapshot
        //Texture2D snapshot = TakeSnapshot(backgroundColor, width, height);
        Texture2D snapshot = capture(width, height);
    ...
    /// <returns>A Texture2D containing the captured snapshot.</returns>
    ///
    public Texture2D capture(float width, float height)
    {
        Camera lCamera = cam;
        Texture2D lOut;
        Rect pRect = new Rect(0, 0, width, height);
    ...
    private Texture2D TakeSnapshot(Color backgroundColor, int width, int height)
    {
        ...
        for (int x = 0; x < texture.width; ++x)
        {
            ...
            {
                Color lColorWhenBlack = texture.GetPixel(x, y);
                Color lColorWhenWhite = texture.GetPixel(x, y);
                if (lColorWhenBlack != Color.clear)
                {
                    //set real color
        ...
        RenderTexture.ReleaseTemporary(previouslyActiveRenderTexture);

        if (RenderTexture.active != null)
        {
            RenderTexture.active.Release();
        }
        if (previouslyActiveRenderTexture != null)
            previouslyActiveRenderTexture.Release();
        if (cam.targetTexture != null)
            cam.targetTexture.Release();
        RenderTexture.active = null;
        cam.targetTexture = null;
        previouslyActiveRenderTexture = null;
